...
const useScanFrontDocument = (
  ...
  const [croppedMugshotImageData, setCroppedMugshotImageData] =
    useState<any>(null);
  ...
  const documentCallback = (result: any) => {
    ...
    const { op_status } = result.returnValue;
    setResultResponse(result.returnValue);
    setResultStatus(op_status);
    if (
  ...
  const faceCompareCallback = async (result: any) => {
    console.log("Face Compare Result?", result);
  ...
  useEffect(() => {
    if (
      ...
      console.log({ enrollImageData,
        croppedDocument,})
      const doCompare = async () => {
        await faceCompareImage(
          croppedDocument,
          enrollImageData,
          faceCompareCallback,
          {
  ...
  const scanFrontDocument = async (
    ...
    const result: any = await scanFrontDocumentId(
